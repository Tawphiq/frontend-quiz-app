...
const Quiz: React.FC = () => {
      ...
      switch (event.key) {
        case 'ArrowDown':
          setFocusedIndex((prevIndex) =>
            prevIndex === null || prevIndex === questions[currentQuestionIndex].options.length - 1
              ? 0
              : prevIndex + 1
          );
          break;
        ...
        case 'Enter':
          if (focusedIndex !== null) {
            setSelectedOption(questions[currentQuestionIndex].options[focusedIndex]);
            handleAnswer();
          }
          break;
      }
  ...
  return (
    <div className="p-4 md:p-20" onKeyDown={handleKeyDown} tabIndex={0}>
      {!quizCompleted ? (
        <div className="lg:flex lg:gap-20">
          <div className="lg:w-1/2">
            <i className="mb-4 text-center lg:text-left font-thin text-sm md:text-2xl text-darkthin dark:text-thinlight">
              ...
              {questions[currentQuestionIndex].options.map((option, index) => {
                ...
                return (
                  <button
                    ...
                    className={`group p-4 h-20 rounded-2xl shadow bg-white dark:bg-dark md:text-xl font-semibold text-dark dark:text-white w-full text-left mb-2 flex items-center justify-between ${
                      focusedIndex === index ? 'border-4 border-purple-500' : ''
                    } ${
                      showAnswer
                        ? isCorrect
                          ? `${notSelectedButCorrect ? '' : 'border-green border-2'}`
